...
(function () {
  ...
  const G = window.GEO;
  const COLOR = { alta: '#24586a', media: '#7fadb3', baja: '#cfe0e3', none: '#e9f0ef' };
  let paths = '';
  G.context.forEach((code) => { paths += `<path d="${G.shapes[code]}" fill="#d9e4e3" stroke="#fff" stroke-width=".7"/>`; });
  G.lac.forEach((code) => {
  ...
  /* ---- bar chart ---- */
  const shades = ['var(--map-alta)', '#35707b', '#4a8d89', '#7fadb3', '#9fc4c0', '#cfe0e3'];
  ...
  let rects = '';
  for (let y = 0; y < N; y++) for (let x = 0; x < N; x++) {
    if (grid[y][x]) rects += `<rect x="${x}" y="${y}" width="1" height="1" fill="#223f4a"/>`;
  }
